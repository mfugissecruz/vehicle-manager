    ...
    <div class="px-4 py-2.5">
        <div class="shadow-md sm:rounded-lg">
            <div class="flex flex-col md:flex-row items-center justify-between gap-2.5 p-4 bg-white">
                ...
                <div>
                    <a href="{{ route('dashboard.drivers.create') }}" class="flex items-center justify-center gap-1 px-4 py-2 text-white bg-indigo-500 rounded hover:bg-indigo-600">
                        <span>Adicionar</span>
                        <span>
                        ...
                        @if (count($drivers))
                            @foreach ($drivers as $key => $driver)
                                <tr class="{{ $key % 2 === 0  ? '' : 'bg-gray-50'}}">
                                    <td class="px-6 py-4">
                                        <img src="{{ asset($driver->image) }}"  alt="{{ $driver->name }}" width="" class="h-auto rounded-full w-14" />
                                    </td>
                                    ...
                                    <td class="px-6 py-4 space-x-3">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('dashboard.drivers.edit', $driver->id) }}" class="font-medium text-blue-600 hover:underline">Editar</a>
                                            <button onclick="confirmDelete('{{ route('dashboard.drivers.destroy', $driver->id) }}')" class="font-medium text-red-600 hover:underline">Deletar</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
