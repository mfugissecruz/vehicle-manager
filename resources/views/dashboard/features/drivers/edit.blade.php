<x-dashboard.default-layout>
    <div class="flex flex-col items-center justify-start max-w-2xl gap-4 px-8 py-4 m-4 mx-auto bg-white rounded shadow-md">
        <div>
            <img src="{{ asset($driver->image) }}" alt="{{ $driver->model }}" class="w-48 h-auto rounded-full">
        </div>
        <form action="{{ route( 'dashboard.drivers.update', $driver->id) }}" method="POST" enctype="multipart/form-data" class="w-full overflow-y-auto">
            @csrf
            @method('PUT')

            {{-- title --}}
            <div class="mb-4">
                <h1 class="text-2xl font-bold">Editar Dados do Motorista</h1>
            </div>
            ...
            <div class="mb-4">
                <label for="name" class="flex flex-col">
                    <span>Nome</span>
                    <input type="text" name="name" id="name" class="rounded" value="{{ old('name', $driver->name) }}" />
            ...
            <div class="mb-4">
                <label for="cpf" class="flex flex-col">
                    <span>CPF</span>
                    <input x-mask="999.999.999-99" type="text" name="cpf" id="cpf" class="rounded" value="{{ old('cpf', $driver->cpf) }}" />
            ...
            <div class="mb-4">
                <label for="email" class="flex flex-col">
                    <span>Email</span>
                    <input type="email" name="email" id="email" class="rounded" value="{{ old('email', $driver->email) }}" />
            ...
            <div class="mb-4">
                <label for="phone" class="flex flex-col">
                    <span>Telefone</span>
                    <input x-mask="(99) 99999-9999" type="text" name="phone" id="phone" class="rounded" value="{{ old('phone', $driver->phone) }}" />
            ...
            <div class="mb-4">
                <label for="cnh" class="flex flex-col">
                    <span>CNH</span>
                    <input x-mask="99999999999" type="text" name="cnh" id="cnh" class="rounded" value="{{ old('cnh', $driver->cnh) }}" />
            ...
            <div class="mb-4">
                <label for="cnh_category" class="flex flex-col">
                    ...
                    <select name="cnh_category" id="cnh_category" class="rounded" required>
                        ...
                        @foreach ($cnh_categories as $cnh_category)
                            <option value="{{ $cnh_category->value }}" {{ old('cnh_category', $driver->cnh_category->value) == $cnh_category->value ? 'selected' : '' }}>{{ $cnh_category->name }}</option>
                        @endforeach
                    </select>
             ...
             <div class="mb-4">
                <label for="cnh_expiration_date" class="flex flex-col">
                    ...
                    <input type="date" name="cnh_expiration_date" id="cnh_expiration_date" class="rounded" value="{{ old('cnh_expiration_date', \Carbon\Carbon::parse($driver->cnh_expiration_date))->format('Y-m-d') }}" />
