    ...
    <div class="px-4 py-2.5">
        <div class="shadow-md sm:rounded-lg">
            <div class="flex items-center justify-between p-4 overflow-y-auto bg-white">
                ...
                <div>
                    <div class="h-6"></div>
                    <a href="{{ route('dashboard.fuel-supply-records.create') }}" class="px-5 text-white bg-indigo-500 hover:bg-indigo-600 py-2.5 rounded-md">
                        Adicionar
                    </a>
                </div>
            </div>
            <div class="overflow-x-auto shadow-md">
                <table class="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        @if (count($fuelSupplyRecords))
                            @foreach ($fuelSupplyRecords as $key => $fuelSupplyRecord)
                                <tr class="{{ $key % 2 === 0  ? 'bg-gray-200' : ''}}">
                                    <td class="px-6 py-4 text-base">
                                        {{ $fuelSupplyRecord->vehicle->model }}
                                    </td>

                                    <td class="px-6 py-4 text-base">
                                        {{ \Carbon\Carbon::parse($fuelSupplyRecord->date)->format('d/m/Y')}}
                                    </td>

                                    <td class="px-6 py-4 text-base">
                                        {{ $fuelSupplyRecord->fuel_type->name }}
                                    </td>
                                    <td class="px-6 py-4 text-base">
                                        {{ str_replace('.', ',', strval($fuelSupplyRecord->fuel_quantity / 1000)) }} L
                                    </td>

                                    <td class="px-6 py-4 text-base">
                                        {{ $fuelSupplyRecord->mileage }} km
                                    </td>
        ...
        <div class="my-5">
            {{ $fuelSupplyRecords->appends(['start_date' => request('start_date'), 'end_date' => request('end_date')])->links() }}
        </div>
    </div>
</x-dashboard.default-layout>
