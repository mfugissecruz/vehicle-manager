<x-dashboard.default-layout>
    <div class="flex flex-col items-center justify-start max-w-2xl gap-4 px-8 py-4 m-4 mx-auto bg-white rounded shadow-md">
        <form action="{{ route('dashboard.fuel-supply-records.store') }}" method="POST" enctype="multipart/form-data" class="w-full overflow-y-auto">
            @csrf
            <div class="mb-4">
                <h1 class="text-2xl font-bold">Registro de Abastecimento</h1>
            </div>
                ...
                <label for="fuel_type" class="flex flex-col">
                    ...
                    <select name="fuel_type" id="fuel_type" class="rounded" required>
                        ...
                        @foreach ($fuel_types as $fuel_type)
                            <option value="{{ $fuel_type->value }}" {{ old('fuel_type') == $fuel_type->value ? 'selected' : '' }}>{{ ucfirst($fuel_type->name) }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-4">
                <label for="fuel_quantity" class="flex flex-col">
                    ...
                    <input type="text" name="fuel_quantity" id="fuel_quantity" class="rounded" value="{{ old('fuel_quantity') }}" />
