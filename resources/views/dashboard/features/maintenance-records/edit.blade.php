...
<x-dashboard.default-layout>
    <div class="flex flex-col items-center justify-start max-w-2xl gap-4 px-8 py-4 m-4 mx-auto bg-white rounded shadow-md">
        <form action="{{ route('dashboard.maintenance-records.update', $maintenanceRecord->id) }}" method="POST" enctype="multipart/form-data" class="w-full overflow-y-auto">
            @csrf
            <div class="mb-4">
                <h1 class="text-2xl font-bold">Editar Registro de Manutenção</h1>
            </div>
                ...
                <label for="vehicle_id" class="flex flex-col">
                    ...
                    <select name="vehicle_id" id="vehicle_id" class="rounded" required>
                        ...
                        @foreach ($vehicles as $vehicle)
                            <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $maintenanceRecord->vehicle_id) == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->model }} - {{ $vehicle->plate_number }}</option>
                        @endforeach
                    </select>
                ...
                <label for="mechanic_id" class="flex flex-col">
                    ...
                    <select name="mechanic_id" id="mechanic_id" class="rounded" required>
                        ...
                        @foreach ($mechanics as $mechanic)
                            <option value="{{ $mechanic->id }}" {{ old('mechanic_id', $maintenanceRecord->mechanic_id) == $mechanic->id ? 'selected' : '' }}>{{ $mechanic->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-4">
                <label for="description" class="flex flex-col">
                    <span>Descrição</span>
                    <textarea rows="5" name="description" id="description" class="rounded" required>{{ old('description', $maintenanceRecord->description) }}</textarea>
            ...
            <div class="mb-4">
                <label for="cost" class="flex flex-col">
                    <span>Custo</span>
                    <input type="text" name="cost" id="cost" class="rounded" value="{{ old('cost', ($maintenanceRecord->cost / 100)) }}" required />
